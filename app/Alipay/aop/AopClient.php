...
class AopClient {
	...
	public function sdkExecute($request) {
		...
		$this->setupCharsets($request);//设置字符集
		...
		$version = $request->getApiVersion();//获取版本号
...
/*echo "<pre>";
		...
		$params['sign'] = null;
		return $this->verify($this->getSignContent($params), $sign, $rsaPublicKeyFilePath,$signType);//支付宝官方说明中异步通知需要做url_encode();
		//return $this->verify($this->getSignContentUrlencode($params), $sign, $rsaPublicKeyFilePath,$signType);
	}
