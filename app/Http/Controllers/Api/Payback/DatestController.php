...
class DatestController extends Controller
{
    ...
    public function setParamsForAliPay(Request $request)
    {
       echo "teaaa";die;
        ...
        //生成请求参数
        $clientData = $alipayobj->getClientData($request->all());
        if (!empty($clientData)) {
    ...
    public function alipayNotify(Request $request)
    {
        ...
        //获取订单信息
        $orderInfo = Order::instance()->getOrderById($alipayData['out_trade_no']);
        //获取用户信息
        ...
        if ($flag && $queryData['trade_status'] == 'TRADE_SUCCESS') {
            DB::beginTransaction();
           ...
           // $jpushObj = new JpushController();
            ...
            //订单相关参数检查
            if ((empty($orderInfo) || $orderInfo['pay_fee'] != $alipayData['total_amount'] || $alipayData['app_id'] != '2017071907808204')) {
            ...
            $orderData['payed_at'] = strtotime($alipayData['notify_time']);
            $orderData['payment_id'] = 1;
            $orderData['is_pay'] = 1;//已付款
            $orderData['pay_type'] = 0;
            $orderData['updated_at'] = strtotime($alipayData['notify_time']);
            $result1 = Order::instance()->where("order_id", $alipayData['out_trade_no'])->update($orderData);*/
            ...
            $dataOrderLog['order_id'] = $alipayData['out_trade_no'];
            $dataOrderLog['type'] = 1;//操作类型 1支付 2退款
            $dataOrderLog['pay_type'] = 1;//支付渠道 1支付宝 2微信
            $dataOrderLog['data'] = serialize($alipayData);
            $dataOrderLog['created_at'] = strtotime($alipayData['notify_time']);
            ...
            //订单状态走向表
            /*$orderRouteData = [];
            $orderRouteData['order_id'] = $alipayData['out_trade_no'];
            ...
            if (/*$result1 &&*/ $result2 && $result3/* && $result4*/ && ($alipayData['trade_status'] == 'TRADE_FINISHED' || $alipayData['trade_status'] == 'TRADE_SUCCESS')) {
                DB::commit();
                // 记帐
                ...
                $alipayData = array_merge($alipayData, array('result1' => $result1, 'result2' => $result2, 'result3' => $result3, 'result4' => $result4));
    ...
    public function alipayReturnUrl(Request $request)
    {
        ...
        //检查订单
        $orderInfo = Order::instance()->getOrderById($responData['result']['alipay_trade_app_pay_response']['out_trade_no']);
        if (empty($orderInfo)) {
     ...
     * @return void
     */
    public function recoveryData(Request $request)
    {
        CanteenAccount::instance()->recoveryDataByArray($request->input("year"), $request->input("month"), $request->input("day"));
    }

    /**手动添加指定日期数据到canteen_account canteen_account_daydata_log表
     * $param year 年
     * $param month 月
     * $param day 日
     * $param canteen_id 小卖部id
     */
    public function canteen_account(Request $request)
    {
        CanteenAccount::instance()->manalinsertData($request->input("year"), $request->input("month"), $request->input("day"), $request->input("canteen_id"));
    }

    /**@Author: YanLong-Rui
     *  @Modifications:双十一刷单活动，自动刷单功能
     *  @date:2017/9/1
     $order_no int 订单号
     * $user_id int 用户id
     * @return void
     */
    public function publishOrders($order_no){
        try {
            //获取订单信息
            ...
            //修改订单
            $orderData['order_status'] = 20;
            $orderData['payed_at'] = time();//strtotime($alipayData['notify_time']);
            $orderData['payment_id'] = 1;
            $orderData['is_pay'] = 1;//已付款
            $orderData['pay_type'] = 0;
            $orderData['updated_at'] = time();//strtotime($alipayData['notify_time']);
            $result1 = Order::instance()->where("order_id", $order_no)->update($orderData);
            if(!$result1){
                throw new \Exception("修改订单表失败！");
            }
            //订单状态走向表
            $orderRouteData = [];
            $orderRouteData['order_id'] = $order_no;
            $orderRouteData['order_status'] = 20;
            $orderRouteData['create_time'] = time();
            $result4 = DB::table('order_route')->insert($orderRouteData);

            if(!$result4){
                throw new \Exception("添加order_route表失败！");
            }
                return true;
        }catch(\Exception $e){
            //记录文件日志
            Blogger::__callStatic("notice", ['异常日志.', [['msg'=>$e->getMessage()]], 'logs/publishOrders_exception', date('Y-m-d')]);
            return false;
        }
    }
